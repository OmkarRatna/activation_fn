...
## Define cost groups (clock-clock only. We can also define clock-output, input-clock, input-output)
  ...
  define_cost_group -name I2C -design sig_12_hw
  define_cost_group -name C2O -design sig_12_hw
  define_cost_group -name C2C -design sig_12_hw
...
define_cost_group -name I2O -design sig_12_hw
path_group -from [all::all_inps]  -to [all::all_outs] -group I2O -name I2O
foreach cg [find / -cost_group *] {
  report timing -cost_group [list $cg] >> ../tanhx_report/reports/area_opt/sig_12_hw_all_pretim.rpt
}
...
#report datapath > ./reports/area_opt/sig_12_hw_datapath.rpt
...
foreach cg [find / -cost_group *] {
  report timing -cost_group [list $cg] >> ../tanhx_report/reports/area_opt/sig_12_hw_post_opt.rpt
}
...
write_sdc > ../tanhx_report/reports/area_opt/sig_12_hw_sdc.sdc  
