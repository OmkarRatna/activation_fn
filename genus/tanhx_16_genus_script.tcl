...
## Define cost groups (clock-clock only. We can also define clock-output, input-clock, input-output)
  ...
  define_cost_group -name I2C -design tanhx_16_hw
  define_cost_group -name C2O -design tanhx_16_hw
  define_cost_group -name C2C -design tanhx_16_hw
...
define_cost_group -name I2O -design tanhx_16_hw
path_group -from [all::all_inps]  -to [all::all_outs] -group I2O -name I2O
foreach cg [find / -cost_group *] {
  report timing -cost_group [list $cg] >> ../tanhx_report/reports/tanhx_16_hw_all_pretim.rpt
}
...
#report datapath > ./reports/tanhx_16_hw_datapath.rpt
...
foreach cg [find / -cost_group *] {
  report timing -cost_group [list $cg] >> ../tanhx_report/reports/tanhx_16_hw_post_opt.rpt
}
...
write_sdc > ../tanhx_report/reports/tanhx_16_hw_sdc.sdc  
